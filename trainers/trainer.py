...
class cls_tripletTrainer:
    ...
    def train(self, epoch, ps_manager, data_loader=None):
        ...
        for i in range(nums_batch):
            pedes_x, pedes_y=ps_manager.get_batchData_pedes(i,self.opt.nums_pedes)
    ...
    def _forward(self):
        feature= self.model(self.data)   #用于三元组损失和softmax损失
        self.loss = self.criterion(feature, self.target)  #输出向量、输出得分、目标
    ###end
